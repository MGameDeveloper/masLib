...
#define MAS_DECL_STRUCT_LIST(Name, ...)\
    const char* Name[] = { __VA_ARGS__ };
#define MAS_STRUCT_LIST(List)\
    List, sizeof(List)/sizeof(List[0])

void mas_object_add_structs(uint64_t obj, const char** Structs, uint32_t Count)
{
...
void test()
{
	uint64_t node = mas_object();

	MAS_DECL_STRUCT_LIST(Transform,
		MAS_STRUCT(masPosition),
		MAS_STRUCT(masRotation),
		MAS_STRUCT(masScale));
	mas_object_add_structs(node, MAS_STRUCT_LIST(Transform));
}