...
void* mas_mmap_read(mas_mmap_t* file, size_t offset);
...
bool mas_mmap_write(mas_mmap_t* file, const void* data, size_t size)
{
    ...
    void* after_write   = MAS_PTR_OFFSET(void, file->view_base, file->write_offset + size);
    if(after_write > write_boundry)
    {
...
void* mas_mmap_read(mas_mmap_t* file, size_t offset)
{
    if (!mas_internal_is_mmap_valid(file) || offset > file->view_size)
        return NULL;
